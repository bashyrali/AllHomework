...
public class Main {
    ...
    private static void initRoutes(HttpServer server){
        ...
        server.createContext("/white_rabbit", Main::handleRequest);
    }
    ...
    private static void handleRequest(HttpExchange exchange){
       if (exchange.getRequestURI().toString().equals("/")){
           try{
               ...
               try (PrintWriter writer = getWriterFrom(exchange)) {
                   ...
                   String ctxPath = exchange.getHttpContext()
                           .getPath();
                   write(writer, "<h1>Follow the white rabbit</h1>", method);
       ...
       else if (exchange.getRequestURI().toString().equals("/white_rabbit")){
           try{
               exchange.getResponseHeaders()
                       .add("Content-type", "text/html; charset=UTF-8");
               int responseCode = 200;
               ...
               try (PrintWriter writer = getWriterFrom(exchange)) {
                   ...
                   String ctxPath = exchange.getHttpContext()
                           .getPath();
                   write(writer, "<h1>You are living in the matrix</h1>", method);
                   writer.flush();}
           } catch (IOException e) {
               e.printStackTrace();
           }
           ...
           try{
               ...
               try (PrintWriter writer = getWriterFrom(exchange)) {
                   ...
                   String ctxPath = exchange.getHttpContext()
                           .getPath();
                   write(writer, "<h2>501 Not implemented.</h2>","");
                   writer.flush();}
           } catch (IOException e) {
               e.printStackTrace();
           }
